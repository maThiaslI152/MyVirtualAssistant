...
from ..core.rag_config import CACHE_CONFIG
...
class RAGCache:
    def __init__(self):
        ...
        self.warmup_executor = ThreadPoolExecutor(
            max_workers=CACHE_CONFIG.CACHE_WARMUP_WORKERS
        )
    ...
    def _compress(self, data: Any) -> bytes:
        ...
        if len(serialized) > CACHE_CONFIG.CACHE_COMPRESSION_THRESHOLD:
    ...
    def set(self, key: str, value: Any, ttl: Optional[int] = None, level: str = "memory") -> bool:
        ...
        try:
            ttl = ttl or CACHE_CONFIG.CACHE_TTL
            expires_at = datetime.now() + timedelta(seconds=ttl) if ttl else None
            
            if level == "memory":
                # Check memory limit
                if len(pickle.dumps(self.memory_cache)) > CACHE_CONFIG.CACHE_MAX_MEMORY:
                    # Remove oldest items
                    sorted_items = sorted(
                        self.memory_cache.items(),
                        key=lambda x: x[1]['created_at']
                    )
                    while len(pickle.dumps(self.memory_cache)) > CACHE_CONFIG.CACHE_MAX_MEMORY:
                        if not sorted_items:
                            break
    ...
    async def cache_search_results(
        ...
        key = generate_cache_key("search", query)
